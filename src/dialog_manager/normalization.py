    ...
    if result.name == 'date':
        ...
        if isinstance(date_value, int):
            ...
            if match_weekday:
                ...
                raise NormalizationError('Unknown date value: {}'.format(result.value))

    elif result.name == 'time':
        if intent == Intent.request_schedule:
            time_value = re.sub(r'giờ\s?', '', result.value.lower())
            ...
            if full_time:
                ...
                raise NormalizationError('Unknown time value: {}'.format(result.value))
